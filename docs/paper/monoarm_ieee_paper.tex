...
ground truth end to end. On this pilot sequence (101 frames, single
subject, single camera), MoveNet-Lightning achieved a lower mean
per-joint angle error (29.3\textdegree\ vs.\ 46.6\textdegree) and higher
correlation with ground truth ($r=0.760$ vs.\ $r=0.587$) than MediaPipe.
In diagnosing these results we identify two evaluation pitfalls that
would otherwise bias any framework comparison: (i) naive degree-wise
differencing overstates flexion error by 17--25\% whenever the true
...
produces framework-dependent, oppositely-signed bias. We report both raw
and corrected metrics, quantify the mechanism behind each pitfall, and
discuss the framework's remaining limitations, including the small,
single-subject scale of the real-data pilot presented here.
\end{abstract}
...
This paper makes three contributions. First, we describe MonoArm, a
...
Section~\ref{sec:setup}). Third, in the course of that validation we
identify and quantify two concrete evaluation pitfalls -- an
angle-wraparound artifact and a rotation-observability confound -- that
inflate or bias error metrics in ways specific to Euler-angle
parameterizations of joint orientation, and that we believe generalize
to other monocular joint-angle benchmarking efforts using a similar
decomposition.

We emphasize at the outset that the real-data validation presented here
is a single-subject, single-camera, 101-frame pilot, not a
...
Table~\ref{tab:full} reports metrics over all 101 frames. MoveNet-Lightning
achieved lower MPJAE (29.3\textdegree\ vs.\ 46.6\textdegree), higher
correlation ($r=0.760$ vs.\ $0.587$), higher $R^2$ (0.349 vs.\ 0.006),
and higher PCK@5\textdegree\ (32.4\% vs.\ 8.4\%) than MediaPipe on this
sequence -- the opposite ranking from what the literature-calibrated
synthetic noise profiles used elsewhere in the MonoArm codebase's smoke
tests would predict, underscoring why a synthetic proxy cannot substitute
for real validation. Both frameworks nonetheless retain moderate-to-good
Pearson correlation despite large MAE, which -- combined with the
diagnosis below -- indicates the error is dominated by systematic bias
and parameterization sensitivity rather than uncorrelated tracking noise.

\begin{table}[t]
\centering
\caption{Full-sequence metrics, real CMU Panoptic Studio validation
(101 frames, bilateral ZXY decomposition, right arm shown).
...
\midrule
MPJAE $\downarrow$ (\textdegree)     & 46.56 & \textbf{29.28} \\
RMSE $\downarrow$ (\textdegree)      & 59.69 & \textbf{48.44} \\
Pearson $r$ $\uparrow$               & 0.587 & \textbf{0.760} \\
$R^2$ $\uparrow$                     & 0.006 & \textbf{0.349} \\
PCK@5\textdegree\ $\uparrow$ (\%)    & 8.4   & \textbf{32.4} \\
PCK@10\textdegree\ $\uparrow$ (\%)   & 49.5  & \textbf{64.4} \\
Jitter $\downarrow$ (\textdegree/fr) & \textbf{1.46} & 1.20 \\
\midrule
\multicolumn{3}{@{}l}{\textit{Per-joint MAE (\textdegree)}} \\
\midrule
Shoulder flexion    & 67.76 & \textbf{49.60} \\
Shoulder abduction  & \textbf{14.59} & 15.85 \\
Shoulder rotation   & 75.51 & \textbf{31.99} \\
Elbow flexion       & 28.38 & \textbf{19.69} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
MediaPipe          & 67.78\textdegree & 56.85\textdegree \ ($-16\%$) \\
MoveNet-Lightning  & 49.60\textdegree & 37.37\textdegree \ ($-25\%$) \\
\bottomrule
\end{tabular}
\end{center}
Twelve and nine frames respectively (of 101) are wraparound-affected. The
correction is substantial but not the whole story: even at
56.85\textdegree/37.37\textdegree, flexion remains the least accurate
DOF for both frameworks. We attribute the remainder to the ZXY
decomposition's second singularity, distinct from the arm-hanging pole it
was chosen to avoid: at $\theta_{abd}\!\to\!90^\circ$, both $v_y$ and
$v_z$ in $\theta_{flex}=\operatorname{atan2}(-v_z,-v_y)$ approach zero
simultaneously, so flexion becomes acutely sensitive to small measurement
noise exactly where this pilot sequence's subject spends much of the
clip (Fig.~\ref{fig:sample}; ground-truth abduction exceeds
...
poles -- and we flag \texttt{metrics.py}'s lack of circular-aware
differencing as a concrete, fixable limitation of the current
implementation (Section~\ref{sec:limitations}).

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/timeseries_vs_gt.png}
\caption{Predicted vs.\ ground-truth angle time series over the 101-frame
validation sequence. The ground-truth shoulder-flexion trace's jump
between $+170^\circ$ and $-180^\circ$ (top panel, frames 68--100) is the
$\operatorname{atan2}$ branch-cut artifact discussed in the text; the
ground-truth shoulder-rotation trace held at exactly $0^\circ$ (third
panel, frames 8--67) reflects the rotation-reliability gate.}
\label{fig:timeseries}
\end{figure}

\subsection{Rotation-Reliability Masking}
Table~\ref{tab:cond} restricts evaluation to the 36 of 101 frames where
ground-truth elbow flexion $\geq 25^\circ$ -- the threshold at which
shoulder rotation is geometrically observable
(Section~\ref{sec:angles}). MediaPipe's rotation MAE falls sharply,
75.51\textdegree$\to$17.74\textdegree, consistent with most of its
...
31.99\textdegree$\to$89.75\textdegree. We traced this to a specific,
verifiable mechanism: MoveNet-Lightning's own predicted elbow flexion
crosses the $25^\circ$ reliability threshold in only 20.8\% of frames
(mean predicted elbow flexion 20.7\textdegree\ against a ground-truth
mean of 39.5\textdegree, i.e.\ MoveNet under-estimates elbow bend), so
its rotation estimate defaults to the unreliable placeholder value
$0^\circ$ in 71.3\% of frames regardless of the true angle -- reflected
numerically in the unmasked full-sequence result, where MoveNet's
rotation bias ($-31.99^\circ$) equals its MAE ($31.99^\circ$) to two
decimal places, the signature of a near-constant offset rather than
frame-varying noise. Restricting to frames where ground truth crosses the
threshold selects disproportionately for frames where MoveNet's own
gate has \emph{not} yet triggered, worsening its apparent rotation
error even though the underlying cause -- systematic elbow-flexion
under-estimation -- is a single, consistent failure mode. This is the
clearest illustration in our results of why a reliability flag must be
respected symmetrically by an evaluation harness: masking by
ground-truth reliability alone, without also accounting for each
...
$\geq 25^\circ$, $n=36$ of 101 frames).}
\label{tab:cond}
\begin{tabular}{@{}lcc@{}}
\toprule
Metric & MediaPipe & MoveNet-Lightning \\
\midrule
MPJAE $\downarrow$ (\textdegree)   & \textbf{37.45} & 54.91 \\
Shoulder rotation MAE $\downarrow$ & \textbf{17.74} & 89.75 \\
Shoulder flexion MAE $\downarrow$  & 81.00 & \textbf{65.18} \\
Elbow flexion MAE $\downarrow$     & \textbf{37.31} & 42.10 \\
\bottomrule
\end{tabular}
\end{table}
...
project specification's static-pose stability requirement); neither
framework meets it on this pilot sequence for flexion, rotation, or
elbow flexion, and both fall short on abduction (14.6\textdegree/15.9\textdegree). This
...
real CMU Panoptic Studio ground truth, with the project's
5\textdegree\ target shown as a reference line on the per-joint panels.}
\label{fig:summary}
\end{figure}
...
a single subject, a single camera view, and 101 consecutive frames of one
continuous motion -- sufficient to run the full pipeline end to end on
genuine (non-simulated) data and to surface the two evaluation pitfalls
above, but not a substitute for a multi-subject, multi-action,
statistically powered benchmark. The framework's statistical protocol
(Section~\ref{sec:metrics}) is designed for, and should be re-run on, a
larger sample -- either additional Panoptic Studio sequences and camera
views, or Human3.6M once registration access is available, at which
point the paired significance tests and Holm--Bonferroni correction
become informative rather than nominal.
...
sequence, finding MoveNet-Lightning more accurate than MediaPipe on this
pilot clip (MPJAE 29.3\textdegree\ vs.\ 46.6\textdegree), and, in
diagnosing that result, identified and quantified two evaluation
pitfalls -- angle wraparound and asymmetric reliability masking -- that
we believe are underappreciated hazards for any benchmark built on
Euler-angle joint parameterizations. Extending this pilot to a
statistically powered, multi-subject validation, resolving the PoseNet
runtime incompatibility, and upstreaming the circular-error and
reliability-aware metric corrections into the shipped evaluation
pipeline are the immediate next steps.

\section*{Data and Code Availability}
The CMU Panoptic Studio sequence \texttt{171204\_pose1\_sample} used for
validation is publicly distributed by the dataset's maintainers for
non-commercial research use; the subject is a data-collection participant
included in that public release. All angle-solver, filter, metric, and
evaluation code described in this paper is part of the authors'
open MonoArm codebase.
